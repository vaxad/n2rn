...
async function convertNextToExpo(nextPath, expoProjectName) {
  try {
    ...
    execSync(`npx create-expo-stack@latest ${expoProjectName} --expo-router --tabs --nativewind`, {
      stdio: 'inherit'
    });
...
async function syncDependencies(nextPath, expoProjectName) {
  ...
  if (missingDependencies.length > 0) {
    ...
    execSync(`npm install --prefix ${expoProjectName} ${missingDependencies.map(dep => `${dep}@${webDependencies[dep]}`).join(' ')}`, {
      stdio: 'inherit'
    });
  } else {
...
async function addRNImports(filePath) {
  ...
  if (requiredComponents.size > 0) {
    ...
    const importedComponents = existingImports ? new Set(existingImports[1].split(',').map(comp => comp.trim())) : new Set();
    ...
    if (newComponents.length > 0) {
      ...
      const updatedData = existingImports ? data.replace(existingImports[0], `import { ${[...importedComponents, ...newComponents].join(', ')} } from 'react-native';`) : importStatement + data;
      fs.writeFileSync(filePath, updatedData);
    }
  }
}
